...
static void unpack(void * v, ekf_t * ekf, int n, int m)
{
    ...
    dptr += n;
    ekf->P = dptr;
    dptr += static_cast<long long>(n) * n;
    ekf->Q = dptr;
    dptr += static_cast<long long>(n) * n;
    ekf->R = dptr;
    dptr += m*m;
...
void ekf_init(void * v, int n, int m)
{
    ...
    *ptr = m;
    ...
    /* set T and last_step_is_predict */
    ekf.T0 = 0.0;
    ekf.T1 = 0.0;
    ekf.last_step_is_predict = 0;
}
...
void ekf_set_R(void* v, double R[25])
{
    ...
    /* Copy and rewrite original R with newer R*/
    for (int i = 0; i < 5*5; i++) {
        ekf.R[i] = R[i];
    }
}

int ekf_update_step(void * v, double * z_obs)
{        
    /* unpack incoming structure */
    int * ptr = (int *)v;
    int n = *ptr;
...
int ekf_partial_update_step(void * v, double * z_obs, const int * indexes, const int num_of_indexes)
{
    /* unpack incoming structure */
    int * ptr = (int *)v;
    int n = *ptr;
    ptr++;
    int m = *ptr;  // observable number
    ekf_t ekf;
    unpack(v, &ekf, n, m); 

    double z_obs_filled[5] = {0, 0, 0, 0, 0};
    for (int i = 0; i < 5; i++) {
        z_obs_filled[i] = ekf.hx[i];
    }
    for (int i = 0; i < num_of_indexes; i++) {
        int index = indexes[i];
        z_obs_filled[index] = z_obs[i];
    }
    ...
    /* \hat{x}_k = \hat{x_k} + G_k(z_k - h(\hat{x}_k)) */
    sub(z_obs_filled, ekf.hx, ekf.tmp5, m);
    mulvec(ekf.G, ekf.tmp5, ekf.tmp2, n, m);
    add(ekf.fx, ekf.tmp2, ekf.x, n);
...
{   
    ...
    mulmat(ekf.F, ekf.P, ekf.tmp0, n, n, n);
    transpose(ekf.F, ekf.Ft, n, n);
    mulmat(ekf.tmp0, ekf.Ft, ekf.P, n, n, n);
    accum(ekf.P, ekf.Q, n, n);

    // transfer from original state to new state
    ekf.x = ekf.fx;

    /* change update */
    ekf.last_step_is_predict = 1;
